...
Upstream-бинарь — один статический файл ~40 МБ (демон + CLI + WireGuard + ICE + DNS + файрвол; апстрим уже собирает его без символов, strip ничего не даёт). `NB_COMPRESS=1` упаковывает его через UPX до ~14 МБ: сжатие идёт после проверки SHA256, упакованный файл проверяется запуском до записи на флеш (замер на 0.79.0 штатным upx из Entware: 40.8 -> 14.3 МБ; пакет `upx` ~0.8 МБ ставится сам, после установки его можно удалить — при следующей переустановке скрипт поставит заново). При переустановке на забитом томе старый бинарь удаляется до записи нового (демон к этому моменту уже остановлен, identity в `/opt/var/lib/netbird` не трогается), так что пик занятого места — один файл.
